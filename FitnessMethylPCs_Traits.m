...
clear; clc; close;
%% Export names
exportName = 'PC_Trait_AssociationModel_FonlyMethyl_FonlySites3sig.xlsx';
...
fitnessT = readtable('MethMatrixCG-HumanBloodFitness96manual_20-100.csv', 'VariableNamingRule', 'preserve');
%%%%%%%%%%%%%%%%%%% DONT USE BLOOD METHYLATIONS %%%%%%%%%%%%%%%%%%%%%%%%%%
fitnessSites = fitnessT.Properties.VariableNames(2:end);
fitnessData = table2array(fitnessT(:, 2:end));
samplenames = fitnessT.sampleId;
...
% methylData = fitnessData(convertIdx(samples2convert), :);
...
mId = find(traitIdx ~= 0);
[methylCompsPCA, NEWscores] = pca(fitnessData(mId,fitOnlySites));
...
for j = 1:width(Vblnames)
    ...
    if numel(sig) >= 1 && convertCharsToStrings(trait(:)) ~= "age" && convertCharsToStrings(trait(:)) ~= "sex"
        for p = 1:numel(sig)
            sigPC(n) = sig(p);
            reportData(n,:) = [sigPC(n), mae(sigPC(n)), rSq(sigPC(n)), ageCoef(sigPC(n)), agePval(sigPC(n)), ...
                genderCoef(sigPC(n)), genderPval(sigPC(n)), pcCoef(sigPC(n)), pcPval(sigPC(n)), n];
...
for p = 1:numel(uniqueSigPCs)
    fig = figure(1);
    x = [NEWscores(:,uniqueSigPCs(p)); tomScores(:,uniqueSigPCs(p)); marcoScores(:,uniqueSigPCs(p))];
    l1 = repmat({'Fitness Study'}, height(NEWscores), 1);
    l2 = repmat({'1 '}, height(tomScores), 1);
    % tom
    l3 = repmat({'2 '}, height(marcoScores), 1);
    % Marco
    l = [l1; l2; l3];
    violinplot(x, l);
    ...
    savename = sprintf('Violinplots_PC%d.png', uniqueSigPCs(p));
